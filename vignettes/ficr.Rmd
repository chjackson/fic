...
The following function calculates the focused model comparison statistics for all of these models, for a focus defined by the probability of low birth weight at a set of covariate values supplied as the `vals` argument.
...
ficall <- function(vals) { 
    focus <- function(ests){plogis(ests %*% vals)}
    res <- matrix(nrow=nrow(combs), ncol=9)
    for (i in 1:nrow(combs)){
        XZi <- cbind(X, Z[,which(combs[i,]==1)])
        sub.glm <- glm(low ~ XZi - 1, data=birthwt, family=binomial)
        ficres <- fic(wide=wide.glm, sub=sub.glm, 
                          inds=c(1,1,combs[i,]), inds0=inds0, focus=focus)
        res[i,] <- ficres
    }
    rownames(res) <- rownames(combs)
    colnames(res) <- names(ficres)
    res
}

ress <- ficall(vals.smoke) 
resns <- ficall(vals.nonsmoke) 
```
...
TODO make into a function of the focus, compare smokers vs non. 
...
TODO Vectorise `fic.default` over focus values
