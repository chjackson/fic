...
\hlstd{X} \hlkwb{<-} \hlkwd{rbind}\hlstd{(vals.smoke, vals.nonsmoke)}
\end{alltt}
\end{kframe}
\end{knitrout}
...
Recall that \code{mod2} contains an extra covariate.  For each of the
...
Instead of printing the \code{ficres} object, it is plotted 

Then show how to plot them. TODO condition on ggplot2.  TODO annotate/simplify.

The default \code{plot} method for FIC objects TODO is a simple
...
\hlstd{ress} \hlkwb{<-} \hlkwd{within}\hlstd{(ficres,\{}
  \hlstd{l95} \hlkwb{=} \hlstd{focus} \hlopt{-} \hlkwd{qnorm}\hlstd{(}\hlnum{0.975}\hlstd{)}\hlopt{*}\hlstd{se}
  \hlstd{u95} \hlkwb{=} \hlstd{focus} \hlopt{+} \hlkwd{qnorm}\hlstd{(}\hlnum{0.975}\hlstd{)}\hlopt{*}\hlstd{se}
\hlstd{\})}
\hlstd{ress} \hlkwb{<-} \hlstd{ress[}\hlkwd{order}\hlstd{(ress}\hlopt{$}\hlstd{rmse.adj,} \hlkwc{decreasing}\hlstd{=}\hlnum{TRUE}\hlstd{),]}
\hlstd{ress_wide} \hlkwb{<-} \hlstd{ress[ress}\hlopt{$}\hlstd{mods}\hlopt{==}\hlstr{"11111111"}\hlstd{,]}
\hlstd{wide_est} \hlkwb{<-} \hlkwd{focus}\hlstd{(}\hlkwd{coef}\hlstd{(wide.glm), X)}
\hlstd{ress}\hlopt{$}\hlstd{wide_est} \hlkwb{<-} \hlstd{wide_est[}\hlkwd{match}\hlstd{(ress}\hlopt{$}\hlstd{vals,} \hlkwd{rownames}\hlstd{(wide_est))]}
\hlstd{ps} \hlkwb{<-} \hlkwd{ggplot}\hlstd{(ress[ress}\hlopt{$}\hlstd{mods}\hlopt{!=}\hlstr{"11111111"}\hlstd{,],}
         \hlkwd{aes}\hlstd{(}\hlkwc{x}\hlstd{=focus,} \hlkwc{y}\hlstd{=rmse))} \hlopt{+}
  \hlkwd{facet_grid}\hlstd{(.}\hlopt{~}\hlstd{vals)} \hlopt{+}
  \hlkwd{xlim}\hlstd{(}\hlnum{0}\hlstd{,} \hlnum{0.6}\hlstd{)} \hlopt{+}
  \hlkwd{xlab}\hlstd{(}\hlstr{"Probability of low birth weight"}\hlstd{)} \hlopt{+}
  \hlkwd{ylab}\hlstd{(}\hlstr{"Root mean square error of estimate"}\hlstd{)} \hlopt{+}
  \hlkwd{geom_point}\hlstd{()} \hlopt{+}
  \hlkwd{geom_segment}\hlstd{(}\hlkwd{aes}\hlstd{(}\hlkwc{x}\hlstd{=l95,} \hlkwc{xend}\hlstd{=u95,} \hlkwc{yend}\hlstd{=rmse))} \hlopt{+}
  \hlkwd{geom_point}\hlstd{(}\hlkwc{data}\hlstd{=ress_wide,} \hlkwc{col}\hlstd{=}\hlstr{"red"}\hlstd{)} \hlopt{+}
  \hlkwd{geom_segment}\hlstd{(}\hlkwd{aes}\hlstd{(}\hlkwc{x}\hlstd{=l95,} \hlkwc{xend}\hlstd{=u95,} \hlkwc{yend}\hlstd{=rmse),} \hlkwc{data}\hlstd{=ress_wide,} \hlkwc{col}\hlstd{=}\hlstr{"red"}\hlstd{)} \hlopt{+}
  \hlkwd{geom_text}\hlstd{(}\hlkwd{aes}\hlstd{(}\hlkwc{x}\hlstd{=}\hlnum{0}\hlstd{,} \hlkwc{label}\hlstd{=}\hlstr{"11111111"}\hlstd{,} \hlkwc{hjust}\hlstd{=}\hlnum{0}\hlstd{),} \hlkwc{data}\hlstd{=ress_wide,} \hlkwc{col}\hlstd{=}\hlstr{"red"}\hlstd{,} \hlkwc{size}\hlstd{=}\hlnum{5}\hlstd{)} \hlopt{+}
  \hlkwd{geom_text}\hlstd{(}\hlkwd{aes}\hlstd{(}\hlkwc{x}\hlstd{=}\hlnum{0}\hlstd{,} \hlkwc{label}\hlstd{=mods,} \hlkwc{hjust}\hlstd{=}\hlnum{0}\hlstd{))} \hlopt{+}
  \hlkwd{geom_vline}\hlstd{(}\hlkwd{aes}\hlstd{(}\hlkwc{xintercept} \hlstd{= wide_est),} \hlkwc{col}\hlstd{=}\hlstr{"red"}\hlstd{)} \hlopt{+}
  \hlkwd{theme}\hlstd{(}\hlkwc{text} \hlstd{=} \hlkwd{element_text}\hlstd{(}\hlkwc{size}\hlstd{=}\hlnum{20}\hlstd{),}
        \hlkwc{axis.text.x} \hlstd{=} \hlkwd{element_text}\hlstd{(}\hlkwc{size}\hlstd{=}\hlnum{20}\hlstd{))} \hlopt{+}
  \hlkwd{geom_text}\hlstd{(}\hlkwd{aes}\hlstd{(}\hlkwc{x}\hlstd{=wide_est,} \hlkwc{y}\hlstd{=} \hlnum{0.12}\hlstd{,} \hlkwc{label}\hlstd{=}\hlstr{"Estimate from wide model"}\hlstd{,} \hlkwc{hjust}\hlstd{=}\hlnum{0}\hlstd{),} \hlkwc{col}\hlstd{=}\hlstr{"red"}\hlstd{)}
\hlstd{ps}
\end{alltt}
...
\includegraphics[width=\maxwidth]{fic-unnamed-chunk-11-1} 
